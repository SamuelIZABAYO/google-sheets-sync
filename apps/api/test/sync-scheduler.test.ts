...
describe('sync scheduler', () => {
  ...
  it('marks failed run when enqueue fails and continues with remaining jobs', async () => {
    setRequiredEnv();
    ...
    const userResult = db
      .prepare('INSERT INTO users (email, password_hash) VALUES (?, ?)')
      .run('scheduler-failure@example.com', 'hash');
    const userId = Number(userResult.lastInsertRowid);

    const failingJob = syncJobRepository.create({
      userId,
      name: 'Failing scheduled Job',
      sourceSpreadsheetId: 'sheet-fail',
      destinationType: 'sqlite',
      destinationConfigJson: JSON.stringify({ table: 'records' }),
      fieldMappingJson: JSON.stringify({ id: 'id' }),
      triggerType: 'schedule',
    ...
    const succeedingJob = syncJobRepository.create({
      userId,
      name: 'Succeeding scheduled Job',
      sourceSpreadsheetId: 'sheet-ok',
      destinationType: 'sqlite',
      destinationConfigJson: JSON.stringify({ table: 'records' }),
      fieldMappingJson: JSON.stringify({ id: 'id' }),
      triggerType: 'schedule',
    ...
    const queue = {
      async enqueue(message: { runId: number; jobId: number; userId: number }) {
        if (message.jobId === failingJob.id) {
    ...
    expect(queueMessages).toHaveLength(1);
    expect(queueMessages[0]?.jobId).toBe(succeedingJob.id);

    const failedRuns = syncRunRepository.listByJobForUser(failingJob.id, userId, 5);
    expect(failedRuns).toHaveLength(1);
    expect(failedRuns[0]?.status).toBe('failed');
    expect(failedRuns[0]?.errorMessage).toBe('queue unavailable');

    const successfulRuns = syncRunRepository.listByJobForUser(succeedingJob.id, userId, 5);
    expect(successfulRuns).toHaveLength(1);
    expect(successfulRuns[0]?.status).toBe('queued');
